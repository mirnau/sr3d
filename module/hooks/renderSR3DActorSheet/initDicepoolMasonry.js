...
export function initDicepoolMasonry(app, html, data) {
    ...
    if (!actor.dicepoolSkillsResizeObserver)
        actor.dicepoolSkillsResizeObserver = null; //I assume this has to be destroyed from the actor
    ...
    const masonryResizeConfig = {
        ...
        itemCSSVar: '--dicepool-computed-item-width',
        gutterCSSVar: '--dicepool-gutter-width',
        observer: actor.dicepoolSkillsResizeObserver //keeping the reference so we can reach it from the actor
    };

    actor.dicepoolSkillsResizeObserver = attrObserveMasonryResize(actor, masonryResizeConfig);
}
